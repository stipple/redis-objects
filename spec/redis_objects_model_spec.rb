...
describe Redis::Objects do
  ...
  it "should support class-level increment/decrement of counters" do
    ...
    Roster.increment_redis_objects_counter(:available_slots, @roster.id).should == 11
    Roster.increment_redis_objects_counter(:available_slots, @roster.id, 3).should == 14
    Roster.decrement_redis_objects_counter(:available_slots, @roster.id, 2).should == 12
    Roster.decrement_redis_objects_counter(:available_slots, @roster.id).should == 11
  ...
  it "should support class-level increment/decrement of global counters" do
    ...
    Roster.increment_redis_objects_counter(:total_players_online).should == 1
    Roster.increment_redis_objects_counter(:total_players_online, nil, 3).should == 4
    Roster.decrement_redis_objects_counter(:total_players_online, nil, 2).should == 2
    Roster.decrement_redis_objects_counter(:total_players_online).should == 1
  ...
  it "should take an atomic block for increment/decrement class methods" do
    a = false
    Roster.get_counter(:available_slots, @roster.id).should == 10
    Roster.decrement_redis_objects_counter(:available_slots, @roster.id) do |cnt|
      if cnt >= 0
        a = true
      end
    end
    ...
    Roster.get_counter(:available_slots, @roster.id).should == 8
    Roster.increment_redis_objects_counter(:available_slots, @roster.id) do |cnt|
      if 1 == 2  # should rewind
        true
      end
    end
  ...
  it "should properly throw errors on bad counters" do
    error = nil
    begin
      Roster.increment_redis_objects_counter(:badness, 2)
    rescue => error
    end
